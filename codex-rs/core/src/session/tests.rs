...
async fn built_tools_for_test(
    session: &Session,
    turn_context: &Arc<TurnContext>,
    cancellation_token: &CancellationToken,
) -> CodexResult<Arc<ToolRouter>> {
    let step_context = StepContext::for_test(Arc::clone(turn_context));
    crate::session::turn::built_tools(session, &step_context, cancellation_token).await
}

async fn built_tools_without_exact_tail_hint_for_test(
    session: &Session,
    turn_context: &Arc<TurnContext>,
    cancellation_token: &CancellationToken,
) -> CodexResult<Arc<ToolRouter>> {
    let step_context = StepContext::for_test(Arc::clone(turn_context));
    crate::session::turn::built_tools_without_exact_tail_tool_surface_hint(
        session,
        &step_context,
        cancellation_token,
    )
...
async fn exact_tail_tool_surface_hint_lifecycle() -> anyhow::Result<()> {
    ...
    let ignored =
        built_tools_without_exact_tail_hint_for_test(&session, &turn_context, &cancellation_token)
            .await?;
    assert!(ignored.exact_tail_tool_surface_outcome().is_none());

    let applied = built_tools_for_test(&session, &turn_context, &cancellation_token).await?;
    let outcome = applied
    ...
    let repeated = built_tools_for_test(&session, &turn_context, &cancellation_token).await?;
...
async fn exact_tail_tool_surface_hint_survives_startup_prewarm_tool_build() -> anyhow::Result<()> {
    ...
    let prewarm_router = built_tools_without_exact_tail_hint_for_test(
        &session,
    ...
    let normal_router = built_tools_for_test(&session, &turn_context, &cancellation_token).await?;
    ...
    let repeated_router =
        built_tools_for_test(&session, &turn_context, &cancellation_token).await?;
...
async fn resumed_exact_tail_compaction_reconstructs_tool_surface_hint() -> anyhow::Result<()> {
    ...
    let cancellation_token = CancellationToken::new();
    let router = built_tools_for_test(&session, &turn_context, &cancellation_token).await?;
    let outcome = router
        .exact_tail_tool_surface_outcome()
...
-> anyhow::Result<()> {
    for (route, replacement_history) in [
        ...
        let router =
            built_tools_for_test(&session, &turn_context, &CancellationToken::new()).await?;
        let outcome = router
            .exact_tail_tool_surface_outcome()
...
async fn resumed_exact_tail_compaction_uses_newest_active_tool_surface_hint() -> anyhow::Result<()>
{
    ...
    let router = built_tools_for_test(&session, &turn_context, &CancellationToken::new()).await?;
    let outcome = router
        .exact_tail_tool_surface_outcome()
...
fn tool_surface_diagnostic(compaction_id: &str) -> RolloutItem {
    RolloutItem::EventMsg(EventMsg::ExactTailToolSurfaceDiagnostic(
        codex_protocol::protocol::ExactTailToolSurfaceDiagnosticEvent {
            ...
            missing_tool_references: vec!["missing_tool".to_string()],
            rehydrated_tool_namespaces: Vec::new(),
            missing_tool_rejection_reasons: vec!["not_model_visible".to_string()],
            hot_tool_reference_overflow_count: 0,
