...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn pre_sampling_semantic_compact_falls_back_after_previous_model_invalid_request_on_downshift()
 {
    skip_if_no_network!();
    ...
    assert_eq!(
        requests.len(),
        4,
        "semantic compaction should retry with the smaller model and then sample"
    );
    assert_eq!(
        requests[0].body_json()["model"].as_str(),
        Some(retired_model)
