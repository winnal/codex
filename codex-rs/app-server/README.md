...
- `config/read` — fetch the effective config on disk after resolving config layering, including opaque `desktop` values stored in `config.toml` and prompt-retention fields such as `prompt_retention`, `rolling_context_reserve_percent`, `rolling_context_target_tokens`, `rolling_compaction`, `protected_hot_exact_tokens`, `summary_group_token_cap`, `max_summary_levels`, and `compact_when_level_group_count_gt`. `rolling_compaction = "pairwise"` is the enum selector for pairwise rolling compaction; there is no separate `rolling_compaction_enabled` boolean. Source-level `target_prompt_tokens` maps to `rolling_context_target_tokens`; visible summary fragmentation is bounded internally from `max_summary_levels` and `compact_when_level_group_count_gt`; `high_watermark_tokens` and `background_compaction` are deferred.
...
- `config/batchWrite` — apply multiple config edits atomically to the user's config.toml on disk, with optional `reloadUserConfig: true` to hot-reload loaded threads, including multiple `desktop.*` edits and prompt-retention field updates.
