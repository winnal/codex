...
- `config/read` — fetch the effective config on disk after resolving config layering, including opaque `desktop` values stored in `config.toml` and prompt-retention fields such as `prompt_retention`, `rolling_context_reserve_percent`, and `rolling_context_target_tokens`.
