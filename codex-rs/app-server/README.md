...
- `config/read` — fetch the effective config on disk after resolving config layering, including opaque `desktop` values stored in `config.toml`. The response includes `compact_preserve_recent_tokens` for the matching `config.toml` key; `null` or omission disables exact-tail compaction, while a positive token count makes compaction summarize only the cold prefix and preserve the recent suffix exactly. Exact-tail route selection is exposed as `compact_exact_tail_strategy` (`auto`, `remote_legacy`, `remote_v2`, or `semantic_transcript`). The semantic transcript route also exposes `compact_exact_tail_semantic_transcript_retained_message_token_budget`, which defaults to `32000` and must be between `10000` and `64000` when set.
